...
DEFAULT_CUTOFFS = ["C4", "C6", "C8"]
DEFAULT_MIN_SEQ_SEP_VALUES = [3, 4]

N_ENERGY_PARAMS = 11
N_ENTROPY_PARAMS = len(AA_ORDER)
N_TOTAL_PARAMS = N_ENERGY_PARAMS + N_ENTROPY_PARAMS
N_LEGACY_TOTAL_PARAMS = 4 + N_ENTROPY_PARAMS
...
def get_default_energy_params():
    return np.array([0.0] + [0.1] * 10, dtype=np.float64)


# sasa-related helpers
...
# loading a single protein and building the dataset
...
# core wsme energy and partition function calculations

def build_w_from_coefficients(contact_matrix, vdw_matrix, elec_matrix, entropy_vector, energy_params, R, T):
    ...
    a0 = energy_params[0]
    a1, a2, a3, a4, a5 = energy_params[1:6]
    b1, b2, b3, b4, b5 = energy_params[6:11]
    ...
    pair_energy = contact_matrix * np.clip(energy_term, -3.0, 1.0)
...
# profile smoothing and two-state analysis
...
# single protein target evaluation
...
# entropy handling and regularization
...
def objective_function_global(
    ...
    call_counter["n"] += 1
    energy_params = params[:N_ENERGY_PARAMS]
    entropy_aa_params = params[N_ENERGY_PARAMS:N_ENERGY_PARAMS + N_ENTROPY_PARAMS]
    ...
    if call_counter["n"] % 10 == 0:
        print(
            ...
            f"a0={energy_params[0]:.5f}, a1={energy_params[1]:.5f}, "
            f"b1={energy_params[6]:.5f}, n_valid={n_valid}, n_invalid={len(invalid_ids)}"
        )
...
def save_experiment_summary_csv(path, rows):
    save_csv(path, rows, [
        "cutoff",
        "min_seq_sep",
        "restart_file",
...
# restart parameter parsing and legacy upgrade

def upgrade_legacy_restart_params(values):
    if len(values) != N_LEGACY_TOTAL_PARAMS:
        raise ValueError(f"Expected legacy restart length {N_LEGACY_TOTAL_PARAMS}, got {len(values)}")

    old_a1, old_b1, old_a2, old_b2 = values[:4]
    old_entropy = values[4:]

    upgraded_energy = np.array([
        old_b1 + old_b2,
        old_a1,
        0.0,
        0.0,
    ...
    return np.concatenate([upgraded_energy, np.array(old_entropy, dtype=np.float64)])
...
def parse_restart_params_string(s):
    ...
    if len(values) == N_TOTAL_PARAMS:
        return np.array(values, dtype=np.float64)

    if len(values) == N_LEGACY_TOTAL_PARAMS:
        print("Detected legacy restart file with 24 params. Upgrading it to the new 31-param polynomial model.")
        return upgrade_legacy_restart_params(values)

    raise ValueError(
        f"Expected {N_TOTAL_PARAMS} new-format params or {N_LEGACY_TOTAL_PARAMS} legacy params, got {len(values)}"
    )
...
def find_restart_file_for_combination(restart_dir, cutoff, min_seq_sep):
    pattern = os.path.join(restart_dir, f"restart_params_{cutoff}_sep{min_seq_sep}_maxfev*.txt")
    ...
    def sort_key(path):
        ...
        match = re.search(r"_maxfev(\d+)\.txt$", name)
        maxfev = int(match.group(1)) if match else -1
        return (maxfev, name)
...
def reset_optimization_trackers():
    call_counter["n"] = 0
...
def run_nelder_mead_stage(
    ...
    result = minimize(
        ...
        method="Nelder-Mead",
        options={"maxfev": maxfev, "disp": False, "xatol": 1e-3, "fatol": 1e-3},
    )

    best_params = best_tracker["best_params"] if best_tracker["best_params"] is not None else result.x
    best_loss = best_tracker["best_loss"] if np.isfinite(best_tracker["best_loss"]) else result.fun

    return {
        "method": "Nelder-Mead",
        "result": result,
        "params": np.array(best_params, dtype=np.float64).copy(),
        "loss": float(best_loss),
        "invalid_ids": best_tracker["invalid_ids"][:],
        "invalid_details": best_tracker["invalid_details"][:],
...
def run_single_experiment(
    ...
    if len(x0) != N_TOTAL_PARAMS:
        raise ValueError(f"x0 has length {len(x0)}, expected {N_TOTAL_PARAMS}")
    ...
    nelder_mead_stage = run_nelder_mead_stage(
        x0=x0,
        dataset=dataset,
        smooth_window=smooth_window,
        entropy_l2=entropy_l2,
    ...
    print(
        f"[pid {os.getpid()}] Nelder-Mead done: "
        f"loss={nelder_mead_stage['loss']:.5f}, success={nelder_mead_stage['result'].success}, "
        f"calls={nelder_mead_stage['calls']}"
    )

    final_params = nelder_mead_stage["params"]
    ...
    return {
        ...
        "min_seq_sep": min_seq_sep,
        "params": final_params,
        "final_loss": nelder_mead_stage["loss"],
        "rmse": metrics["rmse"],
        "mae": metrics["mae"],
        "n_valid": metrics["n_valid"],
        "success": nelder_mead_stage["result"].success,
        "message": str(nelder_mead_stage["result"].message),
        "rows": rows,
        "best_invalid_ids": nelder_mead_stage["invalid_ids"],
        "best_invalid_details": nelder_mead_stage["invalid_details"],
        "nelder_mead_loss": nelder_mead_stage["loss"],
        "nelder_mead_success": nelder_mead_stage["result"].success,
        "nelder_mead_message": str(nelder_mead_stage["result"].message),
        "nelder_mead_calls": nelder_mead_stage["calls"],
    }
...
def main():
    ...
    parser.add_argument("--cutoff", default=None, choices=["C4", "C6", "C8"])
    parser.add_argument("--min_seq_sep", type=int, default=None)
    parser.add_argument("--cutoffs", nargs="+", choices=["C4", "C6", "C8"], default=None)
    parser.add_argument("--min_seq_sep_values", nargs="+", type=int, default=None)
    ...
    manual_restart_params = load_restart_params(args.restart_params, args.restart_params_file)

    cutoffs = args.cutoffs if args.cutoffs is not None else (
        [args.cutoff] if args.cutoff is not None else DEFAULT_CUTOFFS
    )
    min_seq_sep_values = args.min_seq_sep_values if args.min_seq_sep_values is not None else (
        [args.min_seq_sep] if args.min_seq_sep is not None else DEFAULT_MIN_SEQ_SEP_VALUES
    )

    restart_dir = args.restart_dir
    if restart_dir is None:
        if args.restart_params_file is not None:
            restart_dir = os.path.dirname(args.restart_params_file)
        else:
            restart_dir = os.getcwd()

    summary_rows = []

    for min_seq_sep in min_seq_sep_values:
        for cutoff in cutoffs:
            combo_restart_file = None
            combo_restart_params = manual_restart_params

            if combo_restart_params is None:
                combo_restart_file = find_restart_file_for_combination(restart_dir, cutoff, min_seq_sep)
                if combo_restart_file is not None:
                    print(f"\nUsing restart file for {cutoff}, sep={min_seq_sep}: {combo_restart_file}")
                    combo_restart_params = load_restart_params(restart_params_file=combo_restart_file)
                else:
                    print(f"\nNo restart file found for {cutoff}, sep={min_seq_sep}. Using default x0.")
            ...
            result = run_single_experiment(
                ...
                restart_params=combo_restart_params,
            ...
            energy_params = params[:N_ENERGY_PARAMS]
            entropy_aa_params_opt = params[N_ENERGY_PARAMS:N_ENERGY_PARAMS + N_ENTROPY_PARAMS]

            print("\n=== GLOBAL SASA FIT RESULT (POLYNOMIAL ENERGY, PER-AA ENTROPY, 1 - NORMALIZED SASA) ===")
            for k, v in [
                ("cutoff", result["cutoff"]),
                ("min_seq_sep", result["min_seq_sep"]),
                ("Nelder-Mead loss", result["nelder_mead_loss"]),
            ...
            print("\n=== ENERGY PARAMETERS ===")
            print("a0 =", energy_params[0])
            for i in range(1, 6):
                print(f"a{i} = {energy_params[i]}")
            for i in range(1, 6):
                print(f"b{i} = {energy_params[5 + i]}")
            ...
            out_csv = f"global_sasa_fit_poly5_results_{cutoff}_sep{min_seq_sep}_maxfev{args.maxfev}.csv"
            save_results_csv(out_csv, result["rows"])
            print(f"\nSaved per-protein results to: {out_csv}")

            restart_out = f"restart_params_poly5_{cutoff}_sep{min_seq_sep}_maxfev{args.maxfev}.txt"
            ...
            summary_rows.append({
                "cutoff": cutoff,
                "min_seq_sep": min_seq_sep,
                "restart_file": combo_restart_file if combo_restart_file is not None else "",
                "nelder_mead_loss": result["nelder_mead_loss"],
                "nelder_mead_success": result["nelder_mead_success"],
                "nelder_mead_message": result["nelder_mead_message"],
                "nelder_mead_calls": result["nelder_mead_calls"],
    ...
    summary_csv = f"global_sasa_fit_poly5_summary_maxfev{args.maxfev}.csv"
    save_experiment_summary_csv(summary_csv, summary_rows)
